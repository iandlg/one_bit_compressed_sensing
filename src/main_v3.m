...
mnist_dir = fullfile("..", "data", "mnist.mat");
...
step = 2;
MNratios = linspace(0,2.5,step);
rows_list = floor(cols * MNratios);
rows_list(1) = 50; 

%% Test on random vector noiseless
% Macros 
M=100;
N=51;
K=5;
...
%% Test noiseless on single image
idx = 18;
...
%% Plot snr and nmse for single noisless image
% Plot SNR
figure(3); clf;
...
legend("OBBCS", "BIHT","OBLP", "OBBP")
grid('on')
output_file_path = fullfile(output_dir, "reconstruction_time_noisy_1mg.png");
exportgraphics(gcf, output_file_path, "Resolution",300);
hold off;
